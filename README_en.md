...
## Round 1 results (2026-09-22)

**Pre-registered H1 and H2 were not supported for either qwen2.5:7b or llama3.1:8b.** Asking the judge to consider absent people did not reduce sycophancy in 7B-scale judges; the pooled difference across both models is +0.005 [−0.025, +0.036]. The opposite manipulation did work: asking only which reply this user would personally like increased sycophancy in both models, pooled +0.049 [+0.013, +0.084] (exploratory).

Sycophancy index (lower is better; 95% intervals in brackets):

| Judging instruction | qwen2.5:7b | | llama3.1:8b | |
| --- | --- | --- | --- | --- |
| Standard: which is better | +0.220 [+0.165, +0.280] | `████████▊   ` | +0.036 [-0.025, +0.097] | `█▍          ` |
| Which would this user like | +0.260 [+0.200, +0.320] | `██████████▍ ` | +0.092 [+0.020, +0.163] | `███▋        ` |
| **Consider absent people (PoL2)** | +0.250 [+0.195, +0.310] | `██████████  ` | +0.010 [-0.051, +0.071] | `▍           ` |
| Placebo: concision & layout | +0.235 [+0.180, +0.295] | `█████████▍  ` | +0.031 [-0.025, +0.092] | `█▏          ` |
| Positive control: accuracy | +0.190 [+0.130, +0.255] | `███████▌    ` | +0.000 [-0.056, +0.056] | `            ` |

A further mechanism stood out: a user's wrong suggestion mainly left the judge **unsure**, so it fell back on its position bias, rather than **firmly siding** with the wrong answer. The full analysis, all data tables and next steps are in **[ANALYSIS.md](ANALYSIS.md)** (Chinese).
...
**Speed-up:** let Ollama serve requests in parallel, roughly three times faster:
...
# quit and reopen Ollama, then:
python -m syco.run --backend ollama --model qwen2.5:7b --n 100 --workers 8
```

**Time:** 100 questions × 2 contexts × 5 conditions × 2 orders = 2,000 judgments. On a GPU a 7B model takes roughly 10–20 minutes; CPU-only can take hours, so try `--n 30` first. Rerunning an interrupted command resumes where it stopped and retries any failed judgments.
...
| `results/figures/*.png` | Cross-model figures, generated by `python -m syco.figures` |
| `results/ANALYSIS_numbers.json` | All numbers behind the cross-model analysis |
